...
const HomeScreen = ({ navigation }) => {
  ...
  const getLocation = (lat, long) => {
    AuthService.ThirtPartyGet(
      ...
      ',' +
      long +
      '&key=AIzaSyDpxo7fM2N3Jge5qYKijThGKfONINqnhoU',
    ).then(res => {
     // console.log('result got from GeoLOcation', res);
      //refSearch.current?.setAddressText(res.plus_code.compound_code);
      //console.log('textSearch', res.plus_code.compound_code);
      //let postcode = res.results[0].address_components[7].long_name;
    //  console.log('postcode', postcode);
      //let data = {
        //code: postcode == undefined ? null : postcode,
       // updated: false,
      //};
  ...
  const getRestaurantDetails = (postcode) => {
            ...
            <View style={styles.inputView}>
              ...
              <GooglePlacesAutocomplete
                ...
                onPress={(data, details) => {
                  ...
                  let postcode = details.address_components[0].long_name; 
                  setPostalCode(postcode); 
                // getRestaurantDetails(postcode);
