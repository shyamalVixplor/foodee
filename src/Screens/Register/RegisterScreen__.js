...
const RegisterScreen = ({navigation, route}) => {
  ...
  const OrderPlace = (userToken, userID) => {
    ...
    let data = {
      restaurant_id:
        RouteData == null ? null : RouteData.productData.restaurantID,
      payment_status: 'pending',
      discount_val: RouteData.productData.discountVal,
      coupon_code: RouteData.coupon_code,
      coupon_val: RouteData.couponVal,
      total_amount: totalPrice,
      amount_payable: RouteData.amountPayable,
      // user_id: userID,
      order_status: 'pending',
      discount_type: RouteData == null ? null : RouteData.discountType,
      service_type: RouteData == null ? null : RouteData.productData.service,
      special_instruction: RouteData.instruction,
      allergy_info: RouteData.allergy,
  ...
  const handleAppleLogin = values => {
    ...
    AuthService.Post('general/appleuser-check', data)
      .then(res => {
        setLoader(false);
        console.log('Apple user response', res);
        console.log('Apple LOGIN User Token=>', res.data.token);
        console.log('Apple LOGIN User ID=>', res.data.user_details.id);
        // return;
        if (res.error == false) {
          authContext.signIn({
            token: res.data.token,
  ...
  const handleRegister = values => {
    ...
    AuthService.Post('user/register-user', data)
      .then(response => {
        ...
        if (response.error == false) {
          let pageData = {
            userData: response.data.userDetail,
         ...
         // navigation.navigate('OtpRegisterScreen', {
          ...
          AlertOccurred('Alert', JSON.stringify(response.message.email), 'ok');
        }
      })
      .catch(error => {
        setLoader(false);
    ...
    <ImageBackground source={bgImage} style={styles.bgImageStyle}>
      <KeyboardAwareScrollView>
        <SafeAreaView style={styles.container}>
          ...
          {/* <Text style={styles.subHeading}>Add your details to Register</Text> */}
          <View>
            <Formik
              innerRef={FormRef}
